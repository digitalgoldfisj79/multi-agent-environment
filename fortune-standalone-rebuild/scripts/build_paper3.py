...
integers that need not be represented at all.
"""
...
SOURCE_MAP = {
    "source_main": {
        "Lemma 2.1": "Lemma 2.1",
        "Theorem 3.1": "Theorem 3.2 (statement repaired: source main `otherwise r(D)=1`; frozen Appendix A.3 correctly has `r(D)<=1`)",
        "unlabelled Section 4 energy decomposition": "Corollary 4.1",
        "Lemma 5.1": "Lemma 5.1",
        "Theorem 6.1": "Theorem 6.1",
        "Corollary 8.1": "Corollary 8.1 (sampling measure defined locally)",
        "Theorem 9.1": "Theorem 9.2",
        "equation (10.1) covariance expansion": "Proposition 10.1",
    },
    "source_appendix_deduplicated": {
        "Lemma A.1": "merged into Lemma 2.1",
        "Lemma A.2": "promoted to Lemma 3.1",
        "Theorem A.3": "authoritative detailed source for Theorem 3.2",
        "Corollary A.4": "promoted to Corollary 4.1",
        "Lemma A.6": "merged into Lemma 5.1 proof",
        "Theorem A.7": "merged into Theorem 6.1 proof",
        "Corollary A.10": "merged into Corollary 8.1",
...
def source_checks() -> dict:
    ...
    if missing:
        raise RuntimeError(f"source markers changed: {missing}")
    return {
        "source_path": str(SRC.relative_to(ROOT)),
        "source_size_bytes": len(src.encode()),
        "main_overstatement_detected": True,
        "appendix_correct_form_detected": True,
    }


def audit(manuscript: str) -> dict:
    forbidden = [
        "Paper I", "Paper II", "Paper III", "companion manuscript",
        "first circulation", "corrected edition", "Correction notice",
        "archived phase", "INT-ISC", "RUHL-FM", "D1-QLINE-NONSAT",
    ]
    ...
    expected = [
        ("Lemma", "2.1"), ("Lemma", "3.1"), ("Theorem", "3.2"),
        ("Corollary", "4.1"), ("Lemma", "5.1"), ("Theorem", "6.1"),
        ("Corollary", "8.1"), ("Lemma", "9.1"), ("Theorem", "9.2"),
        ("Proposition", "10.1"),
    ]
    if labels != expected:
        raise RuntimeError(f"unexpected statement sequence: {labels}")
    cite_keys = set(re.findall(r"@([A-Za-z0-9_.:-]+)", manuscript))
    bib_keys = set(re.findall(r"@[A-Za-z]+\{([^,]+),", REFERENCES))
    missing = sorted(cite_keys - bib_keys)
    if missing:
        raise RuntimeError(f"missing bibliography keys: {missing}")
    return {
        "statement_count": len(labels),
        "statement_sequence": [f"{a} {b}" for a, b in labels],
        "logical_cross_paper_dependencies": 0,
        "appendix_duplicate_statements_retained": 0,
        "fortune_proved": False,
    }


def main() -> None:
    src_meta = source_checks()
    manuscript = MANUSCRIPT.strip() + "\n"
    audit_meta = audit(manuscript)
    OUT.mkdir(parents=True, exist_ok=True)
    (OUT / "manuscript.md").write_text(manuscript, encoding="utf-8")
    (OUT / "references.bib").write_text(REFERENCES.strip() + "\n", encoding="utf-8")
    (OUT / "STATEMENT_MAP.json").write_text(json.dumps(SOURCE_MAP, indent=2) + "\n", encoding="utf-8")

    claim_status = """# Standalone Paper III — claim status

**Publication role:** intrinsic superincreasing pair-sum kernel theory plus an independently derived primorial-centre covariance application.
...
- exact four-prime second-moment expansion.
...
- the Hardy--Littlewood baseline (9.4).

## Open

- the sparse reciprocal non-concentration estimate (8.3);
- the primorial-centre variance estimate (9.6);
- sufficient aggregate four-prime covariance estimates such as (10.3)--(10.4);
- Fortune's conjecture.

## Source repair

The corrected source main text stated in Theorem 3.1 that every non-single-walk nonzero integer difference has `r(D)=1`. This is too strong because an unrepresented integer has `r(D)=0`. The fuller frozen Appendix A.3 already contains the correct statement `r(D)<=1`; the standalone theorem follows that formulation and proves that every *represented* difference outside the sliding family has multiplicity one.
"""
    (OUT / "CLAIM_STATUS.md").write_text(claim_status, encoding="utf-8")

    repro = """# Reproducibility boundary — Paper III

The labelled analytic/combinatorial statements are proved in `manuscript.md` and do not depend on finite computation. The validator is used for independent checks of multiplicity histograms, low-order moments, the exact polynomial identities (7.2)--(7.3), and diagnostic sampling of the tail scale. No finite computation establishes the arithmetic non-concentration target, the prime-pair covariance target, or Fortune's conjecture.
"""
    ...
    manifest = {
        ...
        **src_meta,
        **audit_meta,
        "standalone_path": str((OUT / "manuscript.md").relative_to(ROOT)),
        "source_repair": "main Theorem 3.1 r(D)=1 -> Appendix A.3 r(D)<=1 off sliding family",
        "deduplication": "Appendix A merged into main theorem sequence; no A-numbered duplicate statement retained",
    }
    (OUT / "SOURCE_MANIFEST.json").write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
    print(f"PAPER3_STANDALONE_BUILD_OK statements={audit_meta['statement_count']} words={len(manuscript.split())}")
