...
old = r'''The earlier pair-lift and principal-cancellation reductions identify a bound
...
new = r'''The preceding pair-lift and principal-cancellation analysis supplies the following architectural input:
...
assert text.count(old) == 1
text = text.replace(old, new)
text = text.replace(r'2\sum_{a\ge1}\frac{\mathcal E_a}{m_a}.', r'2\sum_{\substack{a\ge1\\m_a>0}}\frac{\mathcal E_a}{m_a}.', 1)
text = text.replace(r'''\left(\sum_{a\ge1}m_a\right)
...
text = text.replace(r'=\frac12\sum_{a\ge1}\frac{|\Psi_a(L)|^2}{m_a}.', r'=\frac12\sum_{\substack{a\ge1\\m_a>0}}\frac{|\Psi_a(L)|^2}{m_a}.', 1)
text = text.replace(r'\sum_{a\ge1}\frac{\mathcal R_a}{m_a}', r'\sum_{\substack{a\ge1\\m_a>0}}\frac{\mathcal R_a}{m_a}', 1)

old = r'''because the corresponding weighted sum of diagonal terms is \(o(M)\) for the critical prime shell. Uniform control of every small \(a\) is a convenient sufficient condition, not a necessary quantifier.'''
new = r'''because the corresponding weighted sum of diagonal terms is \(o(M)\) for the critical prime shell. Indeed,
\[
\frac{\kappa_{2,a}}{m_a}
...
old = r'''**Proposition 4.3 (one-sided level-set criterion).** Fix \(\varepsilon>0\). It is sufficient to prove, for dyadic
...
new = r'''**Proposition 4.3 (one-sided level-set criterion).** Let \(L(X)\ge1\) satisfy \(L(X)=X^{o(1)}\). It is sufficient to prove, for dyadic
...
old = r'''Moreover, \(m-n\) is even, so its singular series is bounded below by a positive absolute constant.'''
new = r'''Moreover, \(m-n\) is even. For the binary singular series,
\[
