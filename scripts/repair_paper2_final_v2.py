...
assert hashlib.sha256(text.encode()).hexdigest() == '497c5b2a52e8beb93d166b8763646cf03ee315664d31e4d875c6486b3296e22f'
...
replace_once(r'''The earlier pair-lift and principal-cancellation reductions identify a bound
...
as a sufficient local input for the prime-detection architecture. The present paper analyses the exact content of (3.6). The distinction between this sufficient architecture and Theorem 2.4 should be kept explicit: the direct variance theorem is unconditional as an implication, while (3.6) is a harmonic sufficient target arising from the chosen reciprocal-frame reduction.''', r'''The preceding pair-lift and principal-cancellation analysis supplies the following architectural input:
...
Define''')
replace_once(r'2\sum_{a\ge1}\frac{\mathcal E_a}{m_a}.', r'2\sum_{\substack{a\ge1\\m_a>0}}\frac{\mathcal E_a}{m_a}.')
replace_once(r'''\left(\sum_{a\ge1}m_a\right)
\left(\sum_{a\ge1}\frac{|\Psi_a(L)|^2}{m_a}\right)''', r'''\left(\sum_{a\ge1}m_a\right)
\left(\sum_{\substack{a\ge1\\m_a>0}}\frac{|\Psi_a(L)|^2}{m_a}\right)''')
replace_once(r'=\frac12\sum_{a\ge1}\frac{|\Psi_a(L)|^2}{m_a}.', r'=\frac12\sum_{\substack{a\ge1\\m_a>0}}\frac{|\Psi_a(L)|^2}{m_a}.')
replace_once(r'\sum_{a\ge1}\frac{\mathcal R_a}{m_a}', r'\sum_{\substack{a\ge1\\m_a>0}}\frac{\mathcal R_a}{m_a}')

replace_once(r'''because the corresponding weighted sum of diagonal terms is \(o(M)\) for the critical prime shell. Uniform control of every small \(a\) is a convenient sufficient condition, not a necessary quantifier.''', r'''because the corresponding weighted sum of diagonal terms is \(o(M)\) for the critical prime shell. Indeed,
\[
\frac{\kappa_{2,a}}{m_a}
...
replace_once(r'''**Proposition 4.3 (one-sided level-set criterion).** Fix \(\varepsilon>0\). It is sufficient to prove, for dyadic
...
**Proof.** Split \((K_X)_+\) at \(MX^\varepsilon\) and apply the dyadic layer-cake inequality. The low part contributes at most \(MX^\varepsilon\), and each dyadic high level contributes \(MX^{o(1)}\). The logarithmic number of levels is absorbed into \(X^{o(1)}\). \(\square\)''', r'''**Proposition 4.3 (one-sided level-set criterion).** Let \(L(X)\ge1\) satisfy \(L(X)=X^{o(1)}\). It is sufficient to prove, for dyadic
...
replace_once(r'''Moreover, \(m-n\) is even, so its singular series is bounded below by a positive absolute constant.''', r'''Moreover, \(m-n\) is even. For the binary singular series,
\[
