...
def scale(a: list[int], c: int, q: int) -> list[int]: return trim([(c*x)%q for x in a])
def sub(a: list[int], b: list[int], q: int) -> list[int]: return add(a,scale(b,-1,q),q)
...
def rem(a: list[int], f: list[int], q: int) -> list[int]:
    a=trim(a[:]); f=trim(f[:])
    while len(a)>=len(f):
        c=a[-1]*pow(f[-1],-1,q)%q; d=len(a)-len(f)
        for i,x in enumerate(f): a[i+d]=(a[i+d]-c*x)%q
        trim(a)
    return a

def mulmod(a: list[int], b: list[int], f: list[int], q: int) -> list[int]: return rem(mul(a,b,q),f,q)
...
def polynomial(record: list[int], q: int) -> list[int]:
    a2,a1,a0=record; return [a0%q,a1%q,a2%q,1]
...
def eta_frobenius(f: list[int], q: int) -> int:
    x=[0,1]; xq=powmod(x,q,f,q); xq2=powmod(xq,q,f,q)
...
def verify_orbit(row: dict) -> dict:
    ...
    # rho=1 is allowed: zero defect implies rho=1, but the converse is false.
    if rho == 0: raise AssertionError("nonzero scalar rho gate failed")
    if not all(irreducible_cubic(f,q) for f in blocks): raise AssertionError("reducible cubic in true orbit")
    A,B,C,D=blocks
...
def main() -> None:
    parser=argparse.ArgumentParser(); parser.add_argument("panel",type=Path); args=parser.parse_args()
    ...
    for q,summary in summaries.items():
        ...
        if int(summary["incidences"]) != len(rows)*q*(q-1): raise AssertionError(f"AGL orbit count mismatch q={q}")
        rhos=Counter(r["rho"] for r in rows); inverses=Counter(pow(r["rho"],-1,q) for r in rows)
    ...
    print(json.dumps({
        ...
        "rho_one_orbits":sum(r["rho"]==1 for rows in verified.values() for r in rows),
        "checks":[
            "prime-field and canonical affine gauge",
            "nonzero rho, including audited rho=1 points",
