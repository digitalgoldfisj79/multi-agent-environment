...
Every asymptotic assertion presented in this article as proved is established symbolically. Finite enumeration is used only for explicit template constants and independent verification of algebraic identities; the named open estimates are not included in that claim. A self-contained supplementary archive contains the source code, all validator inputs, exact tables, validation outputs, a portable manifest, and a one-command verification runner.
