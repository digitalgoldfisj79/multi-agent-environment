...
It is a zero-diagonal tridiagonal matrix of even size `p-1`. Its determinant has the unique perfect-matching product. Up to sign its exact matching product is

\[
\prod_{j=1}^{(p-1)/2}(2j)(p-2j+1).
\]

Including the determinant sign and reducing modulo `p` gives
...
by Wilson's theorem. Hence the minor is a unit modulo `p`. Therefore
...
4. At the coefficient-matrix stage, a resonance correction requires two elementary generators. The completed Dwork complex may require further generators after the derivative, growth filtration, and Frobenius are included.
...
The next justified Dwork task has a finite two-generator input, but its completed Frobenius closure is not known to be finite or bounded independently of `p`.
...
where the initial coefficient-level resonance term `mathcal R_p` is generated by two endpoint classes. Determine whether completion under the derivative, growth filtration, and Frobenius preserves a bounded generating set, and then project to the exact `mu_3` trace sector.
