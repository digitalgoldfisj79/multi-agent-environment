...
**Status:** the secondary trace has been constructed and its root-cycle quotient geometry identified. A new no-split theorem removes the identity fibre. The function-field `d=1` crown remains open.
...
5. extending the exact prime census without a structural prediction.
...
The secondary-trace construction succeeded and produced two genuine structural theorems. It did not by itself improve the probability of the final nonvanishing theorem as much as hoped: the local term is now an honest positive point count, but point existence is still the arithmetic core.

The route remains viable only through one of:

1. a geometry-specific compactification of `Y_a` with a rational-point theorem and controlled boundary;
...
Without one of those ingredients, the mathematically correct action is to stop this branch rather than take higher cyclotomic moments.
