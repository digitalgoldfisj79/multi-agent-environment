#!/usr/bin/env python3
"""Validate the executed direct d=1 q-line programme and frozen sources."""
...
def main() -> None:
    ...
    assert gates["programme"] == "D1_QLINE_NONSATURATION_WITH_SECONDARY_AIRY_V0_3"
    assert gates["governing_issue"] == 44
    assert gates["parent_application_issue"] == 41
    ...
    assert by_id["QNS-0"]["status"] == "PASS"
    assert by_id["QNS-1"]["status"] == "READY_NEW_THEOREM_REQUIRED"
    assert by_id["QNS-1"]["remote_compute_allowed"] is False
    assert by_id["QNS-2"]["status"] == "BLOCKED_BY_QNS-1"
    assert by_id["ABT-0"]["status"] == "PASS"
    assert by_id["ABT-1"]["status"] == "RAW_INTEGRAL_LANE_CLOSED"
    assert by_id["ABT-VIRTUAL"]["status"] == "SECONDARY_OPEN_NEW_THEOREM"
    assert by_id["ITD-0"]["status"] == "SECONDARY_OPEN_NEW_THEOREM"
    ...
    required = [
        ...
        "QLINE_ROUND0_NONSATURATION_REDUCTION_20260801.md",
    ]
    ...
    print(json.dumps({
        "status": "D1_QLINE_ROUND0_EXACT_PASS",
        ...
        "qns0": by_id["QNS-0"]["status"],
        "active_gate": "QNS-1",
        "secondary_gates": ["ABT-VIRTUAL", "ITD-0"],
